...
const removeBatch = async (req, res) => {
  ...
  const pkValues = ids.map((id: string) => id);
...
const autocomplete = async (req, res) => {
  ...
  res.json(rows.map(row => ({
    key: row[key],
    text: String(text).split(',').map(tk => ({ type: 'string', value: row[tk] })),
  })));
};
...
const getConfig = async (_req, res) => {
  const drizzleTable = tableRefMap['iamUsers'];
  if (!drizzleTable) return res.json(supplement);
  const merged = mergeWithSupplement('iamUsers', drizzleTable as any, supplement);
