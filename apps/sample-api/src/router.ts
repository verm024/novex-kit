...
import * as schemaAudit from './database/schema-audit.ts';
import * as schemaIam from './database/schema-iam.ts';
...
import countryRoute from './country/routes.ts';
import stateRoute from './state/routes.ts';
import studentRoute from './student/routes.ts';
import studentSubjectRoute from './student-subject/routes.ts';
import subjectRoute from './subject/routes.ts';
import usersRoute from './users/routes.ts';

// Generated CRUD routes mounted below — re-run `npm run generate:crud:all` after schema changes
...
export default ({ app }) => {
  app.use(
    ...
    router.use('/student-subject', studentSubjectRoute),
    router.use('/subject', subjectRoute),
    router.use('/users', usersRoute),
  );

  t4tRoutes({ app, routePrefix: '/api/t4t', schema: [schema, schemaIam, schemaAudit] }); // http://127.0.0.1:3000/api/t4t/
